...
var server = 'http://ws-devtools.df.r.appspot.com/';
...
function resetSockets(tab) {
    ...
    getDebuggerTarget().then((target) => {
        if (target.attached) {
            ...
            // 阻止detach，避免两次detach造成的chrome.debugger.attach报错，Unchecked runtime.lastError: Debugger is not attached to the tab with id
            return;
...
app.onRuntimeMessage = function (mesg) {
